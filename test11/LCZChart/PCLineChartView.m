...
{    
    ...
    CGContextAddLineToPoint(ctx, 30, self.frame.size.height-30);
    ...
    for (NSUInteger i=0; i<[self.xLabels count]; i++)
    {
        if (i % numXIntervals == 1 || numXIntervals==1) {
            ...
            CGContextSetLineWidth(ctx, 1);
            CGContextSetRGBStrokeColor(ctx, 0.4f, 0.4f, 0.4f, 0.1f);
            CGContextMoveToPoint(ctx, x, 30);
            CGContextAddLineToPoint(ctx, x, self.frame.size.height-30);
            CGContextStrokePath(ctx);
